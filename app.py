...
def citySearch(city):
    ...
    if len(data) > 0:
        ...
        for key in keys_to_remove:
            for i in range(len(cityWeather)):
                ...
                cityWeather[i]['weather_state_name'] == "Heavy Cloud") :
                    ...
                    # link = link["url"]                    REQUEST IS LOW WHEN UNCOMMENTED
                ...
                cityWeather[i]['weather_state_name'] == "Clear"
                    ...
                    # link = requests.get(link).json()        REQUEST IS LOW WHEN UNCOMMENTED
                    # link = link["file"]
        ...
        for i in range(len(cityWeather)) : 
            ...
            _Weather = cityWeather[i]['weather_state_name']
            _Temp = cityWeather[i]['the_temp']

            del cityWeather[i]['applicable_date']
            del cityWeather[i]['weather_state_name']
            del cityWeather[i]['the_temp']
            cityWeather[i]['Day'] = _date 
            cityWeather[i]['weather'] = _Weather
            cityWeather[i]['temperature'] = _Temp
